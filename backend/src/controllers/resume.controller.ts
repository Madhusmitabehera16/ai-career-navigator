...
import { extractTextFromBuffer, parseResumeText, extractSkillsFromText } from "../services/resume.service";
...
Provide a strict, valid JSON response. Do not output markdown codeblocks (like \`\`\`json ... \`\`\`) or any other conversational text. Return exactly this JSON structure:
{
  ...
  "roadmap": {
    ...
    // 8. Save SkillGap
    const currentSkills = extractSkillsFromText(extractedText);
    ...
    await prisma.resumeImprovement.create({
      data: {
        resumeId: resume.id,
    ...
    const formattedSteps = roadmapSteps.map((step: any, idx: number) => ({
